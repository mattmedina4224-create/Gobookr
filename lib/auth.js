...
function setSessionCookie(res, token) {
  ...
  res.setHeader(
    'Set-Cookie',
    `gobookr_session=${encodeURIComponent(token)}; Max-Age=${maxAge}; Path=/; HttpOnly; SameSite=Lax${cookieSecurityFlag()}`
  );
}

function clearSessionCookie(res) {
  res.setHeader('Set-Cookie', `gobookr_session=; Max-Age=0; Path=/; HttpOnly; SameSite=Lax${cookieSecurityFlag()}`);
}
