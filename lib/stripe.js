...
async function createCheckoutSession({ proId, shopId, email, trialDays, embedded = false }) {
  const priceId = shopId ? stripeShopPriceId() : stripePriceId();
  ...
  if (embedded) {
    ...
    params.return_url = `${baseUrl}${shopId ? '/dashboard/shop/billing' : '/dashboard/pro/billing'}?success=${encodeURIComponent('Payment method saved. Your membership billing is connected.')}&session_id={CHECKOUT_SESSION_ID}`;
  } else {
    params.success_url = `${baseUrl}${shopId ? '/dashboard/shop/billing' : '/dashboard/pro/billing'}?success=${encodeURIComponent('Payment method saved. Your membership billing is connected.')}&session_id={CHECKOUT_SESSION_ID}`;
