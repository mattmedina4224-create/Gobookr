...
async function createCheckoutSession({ proId, email, trialDays, embedded = false }) {
  ...
  if (embedded) {
    ...
    params.return_url = `${baseUrl}/dashboard/pro/billing?success=${encodeURIComponent('Payment method saved. Your membership billing is connected.')}&session_id={CHECKOUT_SESSION_ID}`;
  } else {
    params.success_url = `${baseUrl}/dashboard/pro/billing?success=${encodeURIComponent('Payment method saved. Your membership billing is connected.')}&session_id={CHECKOUT_SESSION_ID}`;
    params.cancel_url = `${baseUrl}/dashboard/pro/billing?error=${encodeURIComponent('Checkout was canceled. No changes were made.')}`;
  }
