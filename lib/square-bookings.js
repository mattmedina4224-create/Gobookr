...
function authorizationUrl(state) {
  ...
  url.searchParams.set('scope', 'APPOINTMENTS_BUSINESS_SETTINGS_READ MERCHANT_PROFILE_READ EMPLOYEES_READ');
...
async function retrieveTeamMember(accessToken, teamMemberId) {
  if (!teamMemberId) return null;
  const data = await squareRequest(`/v2/team-members/${encodeURIComponent(teamMemberId)}`, accessToken);
...
async function listBookableTeamMembers(accessToken, locationId = '') {
  ...
  return Promise.all(profiles.map(async (profile) => {
    let teamMember = null;
    try { teamMember = await retrieveTeamMember(accessToken, profile.team_member_id); } catch (err) {
      console.warn('Could not retrieve Square team member details', profile.team_member_id, err.message);
    }
    const firstName = teamMember && teamMember.given_name ? String(teamMember.given_name).trim() : '';
    const lastName = teamMember && teamMember.family_name ? String(teamMember.family_name).trim() : '';
    return {
      squareTeamMemberId: profile.team_member_id,
      firstName,
      lastName,
      fullName: [firstName, lastName].filter(Boolean).join(' ') || profile.display_name || '',
...
module.exports = { configured, authorizationUrl, makeState, exchangeCode, retrieveTeamMember, listBookableTeamMembers, listLocationBookingProfiles };
