...
class Router {
  constructor() {
    ...
    // Register the dedicated homepage first so it wins over the legacy `/` route.
    require('../routes/home')(this);
  }
    ...
    for (const route of this.routes[method] || []) {
      ...
      if (m) {
        ...
        route.paramNames.forEach((name, i) => {
          const raw = m[i + 1];
          try { params[name] = decodeURIComponent(raw); }
          catch (_) { params[name] = raw; }
        });
