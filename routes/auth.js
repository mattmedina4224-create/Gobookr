...
module.exports = function (router) {
  // Onboarding is registered here so it is available without adding another
  // top-level route registration in server.js.
    ...
    const body = `
      <section class="section container" style="max-width:560px;">
        <div class="panel">
          ...
          <form method="POST" action="/login">
            ...
            <div class="field">
              ...
              <div style="position:relative;">
                <input id="password" type="password" name="password" required style="width:100%;box-sizing:border-box;padding-right:70px;" />
                <button type="button" onclick="const p=document.getElementById('password'); p.type=p.type==='password'?'text':'password';" aria-label="Show or hide password" style="position:absolute;right:38px;top:50%;transform:translateY(-50%);border:0;background:transparent;padding:8px;cursor:pointer;">
          ...
          <p class="muted" style="margin-top:18px;">New to GoBookr? <a href="/signup?role=customer">Create an account</a></p>
        </div>
      </section>`;
        ...
        <div class="panel">
          ...
          <form method="POST" action="/signup">
            ...
            <div class="field">
              <label for="password">Password</label>
              <div style="position:relative;">
                <input id="password" type="password" name="password" minlength="6" required style="width:100%;box-sizing:border-box;padding-right:70px;" />
                <button type="button" onclick="const p=document.getElementById('password'); p.type=p.type==='password'?'text':'password';" aria-label="Show or hide password" style="position:absolute;right:38px;top:50%;transform:translateY(-50%);border:0;background:transparent;padding:8px;cursor:pointer;">
                  <span style="display:block;width:20px;height:20px;position:relative;">
                    <span style="position:absolute;width:16px;height:16px;border:2px solid #555;border-radius:70% 0 70% 0;transform:rotate(45deg);left:2px;top:2px;"></span>
                    <span style="position:absolute;width:6px;height:6px;background:#555;border-radius:50%;left:7px;top:7px;"></span>
            ...
            ${role === 'pro' ? `
              <div class="field"><label for="business_name">Business name</label><input id="business_name" name="business_name" required /></div>
              ...
              <div class="field"><label for="state">State</label><input id="state" name="state" value="CO" required /></div>
            ` : ''}
  ...
  router.post('/signup', async (ctx) => {
    ...
    const result = db.prepare('INSERT INTO users (email, password_hash, role, name, phone) VALUES (?, ?, ?, ?, ?)')
    ...
    if (role === 'pro') {
      ...
      db.prepare(`INSERT INTO pro_profiles
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(userId, businessName || name, category, '', city, state, 0, 0, 0, 'violet', initialsFrom(businessName || name));
    }
