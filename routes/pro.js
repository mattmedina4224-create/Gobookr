...
module.exports = function (router) {
  ...
  router.post('/dashboard/pro/marketing/campaigns', async (ctx) => {
    ...
    try {
      db.prepare('INSERT INTO marketing_campaigns (pro_id, campaign_type, copy, portfolio_item_id, source) VALUES (?, ?, ?, ?, ?)').run(profile.id, campaignType, copy, portfolioItemId, source);
  ...
  router.get('/dashboard/pro/marketing', async (ctx) => {
    ...
    let savedCampaigns = [];
    try { savedCampaigns = db.prepare('SELECT id, campaign_type, copy, created_at FROM marketing_campaigns WHERE pro_id = ? ORDER BY created_at DESC LIMIT 8').all(profile.id); }
