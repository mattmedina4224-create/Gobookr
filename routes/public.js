...
module.exports = function (router) {
  ...
  router.get('/search', async (ctx) => {
    ...
    const searchBar = `<form method="GET" action="/search" class="discovery-search"><input type="hidden" name="type" value="${resultType === 'all' ? '' : escapeHtml(resultType)}" /><input type="hidden" name="category" value="${escapeHtml(category)}" /><input type="hidden" name="city" value="${escapeHtml(city)}" /><input type="hidden" name="radius" value="${radius || ''}" /><input type="hidden" name="lat" value="${hasGps ? lat : ''}" /><input type="hidden" name="lon" value="${hasGps ? lon : ''}" /><span class="discovery-search-icon" aria-hidden="true">⌕</span><input type="search" name="q" maxlength="100" value="${escapeHtml(q)}" placeholder="Search name or business" aria-label="Search name or business" /><button class="btn" type="submit">Search</button></form>`;
    const typeTabs = '<div class="result-type-tabs">' + [['all','All'],['professionals','Professionals'],['businesses','Businesses']].map(([value,label]) => '<a href="' + filterLink({ type: value === 'all' ? '' : value }) + '" class="' + (resultType === value ? 'active' : '') + '">' + label + '</a>').join('') + '</div>';
    const centerLabel = hasGps ? 'your location' : (manualCenter ? manualCenter.label : city);
    const headingSubject = resultType === 'businesses' ? 'Businesses' : (resultType === 'professionals' ? (category ? slugCategory(category) + 's' : 'Professionals') : (category ? slugCategory(category) + 's' : 'Results'));
    const heading = hasRadiusCenter ? `${headingSubject} within ${radius} mile${radius === 1 ? '' : 's'} of ${escapeHtml(centerLabel)}` : `${headingSubject}${city ? ' near ' + escapeHtml(city) : ' near you'}`;
    const body = `<section class="section container discovery-page"><h1 class="find-title">Find a pro</h1>${searchBar}${typeTabs}<form method="GET" action="/search" class="quick-location"><input type="hidden" name="type" value="${resultType === 'all' ? '' : escapeHtml(resultType)}" /><input type="hidden" name="q" value="${escapeHtml(q)}" /><label><span>Location</span><input type="text" name="city" maxlength="100" value="${escapeHtml(city)}" placeholder="City or ZIP" /></label><label><span>Distance</span><select name="radius"><option value="1"${radius === 1 ? ' selected' : ''}>1 mi</option><option value="2"${radius === 2 ? ' selected' : ''}>2 mi</option><option value="3"${radius === 3 ? ' selected' : ''}>3 mi</option><option value="4"${radius === 4 ? ' selected' : ''}>4 mi</option><option value="5"${!radius || radius === 5 ? ' selected' : ''}>5 mi</option><option value="10"${radius === 10 ? ' selected' : ''}>10 mi</option><option value="15"${radius === 15 ? ' selected' : ''}>15 mi</option><option value="20"${radius === 20 ? ' selected' : ''}>20 mi</option></select></label><button class="btn" type="submit">Go</button></form><div class="discovery-summary"><span>${totalResults} result${totalResults === 1 ? '' : 's'}${hasRadiusCenter ? ' · within ' + radius + ' mi' : ''}</span>${(category || city || q || hasRadiusCenter) ? '<a href="/search">Reset</a>' : ''}</div><div class="search-layout"><aside class="filters"><h4>Service</h4><ul>${CATEGORIES.map((c) => `<li><a href="${filterLink({ category: c.value })}" style="display:block;padding:6px 0;font-weight:${c.value === category ? '800' : '500'};color:${c.value === category ? 'var(--brand)' : 'inherit'};">${c.label}</a></li>`).join('')}</ul></aside><div>${totalResults ? `<div class="pro-grid">${results.map(proCard).join('')}${businesses.map(businessCard).join('')}</div>` : `<div class="empty-state"><h3>No results nearby</h3><p>Try a larger distance or another city or ZIP.</p></div>`}</div></div></section>`;
