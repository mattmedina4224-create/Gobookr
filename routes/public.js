...
module.exports = function (router) {
  ...
  router.get('/search', async (ctx) => {
    ...
    const searchBar = `<form method="GET" action="/search" style="display:flex;gap:12px;align-items:stretch;margin:0 0 22px;width:100%;"><input type="hidden" name="type" value="${resultType === 'all' ? '' : escapeHtml(resultType)}" /><input type="hidden" name="category" value="${escapeHtml(category)}" /><input type="hidden" name="city" value="${escapeHtml(city)}" /><input type="hidden" name="radius" value="${radius || ''}" /><input type="hidden" name="lat" value="${hasGps ? lat : ''}" /><input type="hidden" name="lon" value="${hasGps ? lon : ''}" /><input type="search" name="q" maxlength="100" value="${escapeHtml(q)}" placeholder="Search by name or business" aria-label="Search by name or business" style="flex:1;min-width:0;width:100%;height:56px;font-size:17px;padding:0 18px;border-radius:14px;" /><button class="btn" type="submit" style="min-height:56px;padding:0 26px;border-radius:14px;font-size:16px;">Search</button></form>`;
