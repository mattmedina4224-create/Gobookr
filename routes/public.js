...
module.exports = function (router) {
  ...
  router.get('/search', async (ctx) => {
    ...
    let results = db.prepare(sql).all(...args).map(proWithStats).filter(Boolean); if (minRating) results = results.filter((p) => (p.rating || 0) >= Number(minRating));
