...
module.exports = function (router) {
  ...
  router.get('/search', async (ctx) => {
    ...
    const body = `<section class="section container"><div class="section-head"><div><h2>${heading}</h2><p class="muted" style="margin:4px 0 0;">${totalResults} result${totalResults === 1 ? '' : 's'} found${hasRadiusCenter ? ' · sorted by distance' : ''}</p></div>${(category || city || q || hasRadiusCenter) ? '<a class="btn secondary small" href="/search">Clear all filters</a>' : ''}</div>${searchBar}${typeTabs}<div class="search-layout"><aside class="filters"><h4>Service</h4><ul>${CATEGORIES.map((c) => `<li><a href="${filterLink({ category: c.value })}" style="display:block;padding:6px 0;font-weight:${c.value === category ? '800' : '500'};color:${c.value === category ? 'var(--brand)' : 'inherit'};">${c.label}</a></li>`).join('')}</ul><h4 style="margin-top:18px;">City or ZIP</h4><form method="GET" action="/search" class="field"><input type="hidden" name="type" value="${resultType === 'all' ? '' : escapeHtml(resultType)}" /><input type="hidden" name="category" value="${escapeHtml(category)}" /><input type="hidden" name="q" value="${escapeHtml(q)}" /><input type="hidden" name="lat" value="${hasGps ? lat : ''}" /><input type="hidden" name="lon" value="${hasGps ? lon : ''}" /><input type="text" name="city" maxlength="100" value="${escapeHtml(city)}" placeholder="City or ZIP" /><label for="radius-filter" style="display:block;margin-top:12px;font-weight:700;">Distance</label><select id="radius-filter" name="radius" style="width:100%;margin-top:6px;"><option value="1"${radius === 1 ? ' selected' : ''}>1 mile</option><option value="2"${radius === 2 ? ' selected' : ''}>2 miles</option><option value="3"${radius === 3 ? ' selected' : ''}>3 miles</option><option value="4"${radius === 4 ? ' selected' : ''}>4 miles</option><option value="5"${!radius || radius === 5 ? ' selected' : ''}>5 miles</option><option value="10"${radius === 10 ? ' selected' : ''}>10 miles</option><option value="15"${radius === 15 ? ' selected' : ''}>15 miles</option><option value="20"${radius === 20 ? ' selected' : ''}>20 miles</option></select><button class="btn secondary small block" type="submit" style="margin-top:8px;">Apply</button></form></aside><div>${totalResults ? `<div class="pro-grid">${results.map(proCard).join('')}${businesses.map(businessCard).join('')}</div>` : `<div class="empty-state"><h3>No results match those filters</h3><p>Try a larger distance, another city or ZIP code, or another service.</p><a class="btn secondary" href="/search">Clear filters</a></div>`}</div></div></section>`;
