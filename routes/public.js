...
module.exports = function (router) {
  ...
  router.get('/search', async (ctx) => {
    ...
    const hasSearch = Boolean(category || city || q || hasGps);
    const categoryTiles = CATEGORIES.filter((item) => item.value).map((item) => '<a class="service-tile' + (item.value === category ? ' active' : '') + '" href="' + filterLink({ category: item.value, type: resultType === 'businesses' ? '' : resultType }) + '"><span>' + item.label + '</span><span aria-hidden="true">›</span></a>').join('');
    const body = `<section class="discovery-page"><div class="discovery-hero"><div class="container"><div class="discovery-hero-copy"><p class="eyebrow">Beauty & wellness, near you</p><h1>Find your next favorite pro.</h1><p>Discover local barbers, stylists, nail artists, estheticians and more.</p></div><form method="GET" action="/search" class="market-search"><label class="market-field"><span>What do you need?</span><input type="search" name="q" maxlength="100" value="${escapeHtml(q)}" placeholder="Haircut, nails, massage..." /></label><label class="market-field location"><span>Where?</span><input type="text" name="city" maxlength="100" value="${escapeHtml(city)}" placeholder="City or ZIP" /></label><input type="hidden" name="radius" value="${radius || 5}" /><button class="btn market-search-btn" type="submit">Search</button></form></div></div><div class="container discovery-content">${!hasSearch ? `<section class="browse-services"><div class="browse-heading"><div><p class="eyebrow">Explore services</p><h2>What are you looking for?</h2></div></div><div class="service-tiles">${categoryTiles}</div></section>` : `<section class="results-shell"><div class="results-toolbar"><div><a class="back-search" href="/search">‹ New search</a><h2>${resultType === 'businesses' ? 'Businesses' : resultType === 'professionals' ? 'Professionals' : 'Matches'}${city ? ' near ' + escapeHtml(city) : ''}</h2><p>${totalResults} result${totalResults === 1 ? '' : 's'}${hasRadiusCenter ? ' · within ' + radius + ' miles of ' + escapeHtml(centerLabel) : ''}</p></div><div class="results-actions"><div class="result-type-tabs">${[['all','All'],['professionals','Pros'],['businesses','Businesses']].map(([value,label]) => '<a href="' + filterLink({ type: value === 'all' ? '' : value }) + '" class="' + (resultType === value ? 'active' : '') + '">' + label + '</a>').join('')}</div><label class="distance-control"><span>Distance</span><select onchange="window.location.href=this.value">${[1,2,3,4,5,10,15,20].map((m) => '<option value="' + filterLink({ radius:m }) + '"' + (radius === m ? ' selected' : '') + '>' + m + ' mi</option>').join('')}</select></label></div></div>${totalResults ? `<div class="pro-grid">${results.map(proCard).join('')}${businesses.map(businessCard).join('')}</div>` : `<div class="empty-state"><h3>No matches yet</h3><p>Try a nearby city, a broader search, or increase the distance.</p><a class="btn secondary" href="/search">Start a new search</a></div>`}</section>`}</div></section>`;
