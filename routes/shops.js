...
module.exports = function (router) {
  ...
  router.get('/shop/:id/claim', async (ctx) => {
    ...
    const action = pending ? '<div class="panel"><strong>Claim request pending</strong><p class="muted">GoBookr will verify ownership before transferring control of this shop page.</p></div>' :
    ...
    send(ctx.res, layout({ title:`Claim ${shop.name}`, currentUser:ctx.currentUser, session:ctx.session, body:`<section class="section container" style="max-width:720px;"><div class="card"><span class="badge">Unclaimed shop</span><h1>Claim ${escapeHtml(shop.name)}</h1><p class="lede">If you own or manage this business, request control of its GoBookr shop page.</p><div class="panel" style="margin:20px 0;"><strong>${escapeHtml(shop.name)}</strong><div class="muted">${escapeHtml(shop.city)}, ${escapeHtml(shop.state)}</div></div>${action}</div></section>` }));
  });
